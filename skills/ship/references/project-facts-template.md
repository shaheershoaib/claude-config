# Project facts template (for the `ship` loop)
...
name: <project>-loop
description: >-
  Run the <project> work-set pipeline: the project facts (repos, branches,
  verification target, tracker board if any, commit identities, gates) for
  executing the global ship skill on <project> work. Use whenever
  fixing, building, or retesting a <project> work-item - "fix this ticket",
  "fix these", "run the loop", or any batch of <project> asks.
---
...
## Close-out referee (delete if none)
- <the environment's external verification referee, if any - e.g. a Stop hook
  or required CI check that blocks unverified fixed-claims. Name it, where its
  config lives, and any evidence format it expects (e.g. a receipt/marker
